...
if option == 2:
    intervalx = rs.GetReal("intervalx", 500)
else:
    intervalx = rs.GetReal("intervalx", 1000)
    intervaly = rs.GetReal("intervaly", 2000)
Secx = rs.GetReal("mullion width", 150) 
Secy = rs.GetReal("mullion depth", 50) 
# if option == 2:
...
def extframe(srf, vec):
    ...
    direction = rs.CurveTangent(crv, 0)
    newplane = rs.PlaneFromNormal(point, direction, plane.ZAxis)
    frames.append(sweepSec(crv, newplane, vec))
    if crv: rs.DeleteObjects(crv)
    return frames
