...
Edit By willemderks 2019 01 16 to allow a single box for each selected object
Edit By dharman gersch 2019 04 11 to work in grasshopper python component.
    Inputs:
        G: list of geometry to get boundinb box
        N: number of samples. 10="Standard", 18="Fine"
        TODO: get bb for each branch to allow bounding boxes of groups of objects

    Output:
        BB: the resulting minimum volume oriented bounding box"""

__author__ = "Mitch Heynick"
__version__ = "2019.04.12"
__url__ = "https://discourse.mcneel.com/t/minimum-oriented-bounding-box-implementation-in-grasshopper-python-script-node/64344"

ghenv.Component.Name = "minimum Boundinb Box"
ghenv.Component.NickName = 'minbb'
ghenv.Component.Message = ''
ghenv.Component.Category = "Geo"
...
#TODO: add grasshopper inputs for command line options
...
def Min3DBoundingBox(objs,init_plane,count,rel_stop,im_rep):
    ...
    prec=Rhino.RhinoDoc.ActiveDoc.DistanceDisplayPrecision
    us=Rhino.RhinoDoc.ActiveDoc.GetUnitSystemName(True, False, False, False)
...
def MinBoundingRectanglePlane(objs,curr_plane,im_rep=False):
    ...
    if im_rep: print "Initial area: {}".format(curr_area)
    ...
    for i in range(safe):
        ...
        if i==10: print "Max loop limit reached" #debug
...
def CombinedMinBBMulti():
    ...
    inputs = G, False, False, False

    if inputs is None: return
    objIDs, fine_sample, rel_stop, im_rep= inputs
    ...
    for objID in objIDs:
        ...
        if plane:
            ...
            bb.append(Rhino.Geometry.Brep.CreateFromCornerPoints(f_bb[0],f_bb[1],f_bb[2],f_bb[3], 0.1))
